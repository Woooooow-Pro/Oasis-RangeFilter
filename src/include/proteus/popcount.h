...
#define COMPARE_STEP_8(x, y)                                      \
  ((((((x) | MSBS_STEP_8) - ((y) & ~MSBS_STEP_8)) ^ (x) ^ ~(y)) & \
    MSBS_STEP_8) >>                                               \
   7)
#define LEQ_STEP_8(x, y)                                         \
  ((((((y) | MSBS_STEP_8) - ((x) & ~MSBS_STEP_8)) ^ (x) ^ (y)) & \
    MSBS_STEP_8) >>                                              \
   7)

#define UCOMPARE_STEP_9(x, y)                                               \
  (((((((x) | MSBS_STEP_9) - ((y) & ~MSBS_STEP_9)) | (x ^ y)) ^ (x | ~y)) & \
    MSBS_STEP_9) >>                                                         \
   8)
#define UCOMPARE_STEP_16(x, y)                                                \
  (((((((x) | MSBS_STEP_16) - ((y) & ~MSBS_STEP_16)) | (x ^ y)) ^ (x | ~y)) & \
    MSBS_STEP_16) >>                                                          \
   15)
...
inline auto popcountLinear(uint64_t *bits, uint64_t x, uint64_t nbits)
    -> uint64_t {
  if (nbits == 0) {
    return 0;
  }
  ...
  for (uint64_t i = 0; i < lastword;
       i++) { /* tested;  manually unrolling doesn't help, at least in C */
    //__builtin_prefetch(bits + x + i + 3, 0);
    p += popcount(
        bits[x + i]);  // note that use binds us to 64 bit popcount impls
  }
  ...
  uint64_t lastshifted =
      bits[x + lastword] >> (63 - ((nbits - 1) & popcountmask));
  p += popcount(lastshifted);
  return p;
}

// Return the index of the kth bit set in x
inline auto select64_naive(uint64_t x, int k) -> int {
  int count = -1;
  for (int i = 63; i >= 0; i--) {
    count++;
    if (x & (1ULL << i)) {
...
inline auto select64_popcount_search(uint64_t x, int k) -> int {
  int loc = -1;
...
inline auto select64_broadword(uint64_t x, int k) -> int {
  uint64_t word = x;
  int residual = k;
  uint64_t byte_sums;
  ...
  byte_sums =
      (byte_sums & 3 * ONES_STEP_4) + ((byte_sums >> 2) & 3 * ONES_STEP_4);
  ...
  const int place =
      (LEQ_STEP_8(byte_sums, residual_step_8) * ONES_STEP_8 >> 53) & ~0x7;
  ...
  const uint64_t spread_bits =
      (word >> place & 0xFF) * ONES_STEP_8 & INCR_STEP_8;
...
inline auto selectLinear(uint64_t *bits, uint64_t length, uint64_t x,
                         uint64_t k) -> uint64_t {
  if (k > (length - x) * 64) return -1;
  uint64_t i = 0;
