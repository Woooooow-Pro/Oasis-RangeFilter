...
//
// Precondition:  given that Ahigh|Alow represents a 254-bit v_mm_shuffle_epi8alue
...
// hashing the bits in value using the keys key1 and key2 (only the first 64
// bits of key2 are used). This is basically (a xor k1) * (b xor k2) mod p with
// length component
static uint64_t simple128to64hashwithlength(const __m128i value,
                                            const __m128i key,
                                            uint64_t keylength,
                                            uint64_t length) {
  ...
  const __m128i total =
      _mm_xor_si128(clprod1, lazyLengthHash(keylength, length));
...
static __m128i __clmulhalfscalarproductwithoutreduction(
  ...
  // we expect length = 128, so we need  16 cache lines of keys and 16 cache
  // lines of strings.
  if (CLHASH_DEBUG)
...
static __m128i __clmulhalfscalarproductOnlyExtraWord(
    const __m128i *randomsource, const uint64_t extraword) {
  const __m128i temp1 = _mm_load_si128(randomsource);
  const __m128i temp2 = _mm_loadl_epi64((__m128i const *)&extraword);
  const __m128i add1 = _mm_xor_si128(temp1, temp2);
  const __m128i clprod1 = _mm_clmulepi64_si128(add1, add1, 0x01);
...
// there always remain an incomplete word that has 1,2, 3, 4, 5, 6, 7 used
// bytes. we append 0s to it
static inline uint64_t createLastWord(const size_t lengthbyte,
                                      const uint64_t *lastw) {
...
uint64_t clhash(const void *random, const char *stringbyte,
  ...
  const unsigned int m =
      128;  // we process the data in chunks of 16 cache lines
  ...
  __m128i polyvalue = _mm_load_si128(
      rs64 + m128neededperblock);  // to preserve alignment on cache lines for
                                   // main loop, we pick random bits at the end
  polyvalue = _mm_and_si128(
      polyvalue,
      _mm_setr_epi32(0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
                     0x3fffffff));  // setting two highest bits to zero
  // we should check that polyvalue is non-zero, though this is best done
  // outside the function and highly unlikely
  const size_t length = lengthbyte / sizeof(uint64_t);  // # of complete words
  const size_t lengthinc =
      (lengthbyte + sizeof(uint64_t) - 1) /
      sizeof(uint64_t);  // # of words, including partial ones

  const uint64_t *string = (const uint64_t *)stringbyte;
  if (m < lengthinc) {  // long strings // modified from length to lengthinc to
                        // address issue #3 raised by Eik List
    ...
    if (remain != 0) {
      ...
      // there are no completely filled words left, but there is one partial
      // word.
      acc = mul128by128to128_lazymod127(polyvalue, acc);
      const uint64_t lastword = createLastWord(lengthbyte, (string + length));
