...
LoudsSparse::LoudsSparse(const SuRFBuilder* builder) {
  ...
  suffixes_ =
      new BitvectorSuffix(builder->getSuffixes(), num_suffix_bits_per_level,
                          num_suffixes_per_level, start_level_, height_);
}

template <typename T>
auto LoudsSparse::lookupKey(const T& key, PrefixBF* prefix_filter,
  ...
  for (level = start_level_; level < truncated.length(); level++) {
    if (!labels_->search(static_cast<label_t>(truncated[level]), pos,
                         nodeSize(pos))) {
      return false;
    }
    ...
    if (!child_indicator_bits_->readBit(pos)) {
      return (suffixes_->checkEquality(getSuffixPos(pos), truncated, level + 1,
                                       trie_depth_)) &&
...
template <typename T>
auto LoudsSparse::moveToKeyGreaterThan(const T& lq, const T& rq,
  ...
  for (level = start_level_; level < edited_lq.length(); level++) {
    ...
    // if no exact match
    if (!labels_->search(static_cast<label_t>(edited_lq[level]), pos,
                         node_size)) {
...
auto LoudsSparse::serializedSize() const -> uint64_t {
  uint64_t size = sizeof(height_) + sizeof(start_level_) +
                  sizeof(node_count_dense_) + sizeof(child_count_dense_);
  sizeAlign(size);
  size += (labels_->serializedSize() + child_indicator_bits_->serializedSize() +
           louds_bits_->serializedSize() + suffixes_->serializedSize());
  sizeAlign(size);
  return size;
}

auto LoudsSparse::getMemoryUsage() const -> uint64_t {
  return (sizeof(this) + labels_->size() + child_indicator_bits_->size() +
          louds_bits_->size() + suffixes_->size());
}

void LoudsSparse::serialize(char*& dst) const {
...
auto LoudsSparse::deSerialize(char*& src, uint32_t trie_depth) -> LoudsSparse* {
...
auto LoudsSparse::getChildNodeNum(const position_t pos) const -> position_t {
  return (child_indicator_bits_->rank(pos) + child_count_dense_);
}

auto LoudsSparse::getFirstLabelPos(const position_t node_num) const
    -> position_t {
  return louds_bits_->select(node_num + 1 - node_count_dense_);
}

auto LoudsSparse::getLastLabelPos(const position_t node_num) const
    -> position_t {
...
auto LoudsSparse::getSuffixPos(const position_t pos) const -> position_t {
  return (pos - child_indicator_bits_->rank(pos));
}

auto LoudsSparse::nodeSize(const position_t pos) const -> position_t {
...
auto LoudsSparse::compareSuffixGreaterThan(
    const position_t pos, const level_t level, const uint64_t lq,
    const uint64_t rq, std::string& edited_lq, LoudsSparse::Iter& iter,
    PrefixBF* prefix_filter) const -> bool {
  int compare =
...
auto LoudsSparse::compareSuffixGreaterThan(
    ...
    const std::string& rq, std::string& edited_lq, LoudsSparse::Iter& iter,
    PrefixBF* prefix_filter) const -> bool {
  int compare =
      suffixes_->compare(getSuffixPos(pos), edited_lq, level, trie_depth_);
...
LoudsSparse::Iter::Iter(LoudsSparse* trie)
    : is_valid_(false),
      is_done_(false),
      trie_(trie),
      start_node_num_(0),
...
template <typename T>
auto LoudsSparse::Iter::compare(const T& key, PrefixBF* prefix_filter,
  ...
  int suffix_compare = trie_->suffixes_->compare(
      suffix_pos, str_key, start_level_ + iter_key.length(),
      trie_->getTrieDepth());
...
auto LoudsSparse::Iter::getKey() const -> std::string {
...
template auto LoudsSparse::Iter::compare(const uint64_t& key,
                                         PrefixBF* prefix_filter,
                                         const std::string& dense_prefix) const
    -> int;

template auto LoudsSparse::lookupKey(const uint64_t& key,
                                     PrefixBF* prefix_filter,
                                     const position_t in_node_num) const
    -> bool;

template auto LoudsSparse::moveToKeyGreaterThan(const uint64_t& lq,
                                                const uint64_t& rq,
                                                LoudsSparse::Iter& iter,
